...
describe('stripSecrets', () => {
  it('removes apiKey from provider options', () => {
    const cleaned = stripSecrets({
      provider: {
        openai: { options: { apiKey: 'sk-secret', headers: { a: '1' } } },
      },
    })
    expect((cleaned.provider as { openai: { options: { apiKey?: string } } }).openai.options.apiKey).toBeUndefined()
    expect(
      (cleaned.provider as { openai: { options: { headers: Record<string, string> } } }).openai.options.headers.a,
    ).toBe('1')
  })
})
